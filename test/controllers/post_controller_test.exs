defmodule Pxblog.PostControllerTest do
  ...
  test "redirects when trying to edit a post for a different user", %{conn: conn, user: _user, role: role, post: post} do
  ...
  test "updates chosen resource and redirects when data is valid when logged in a admin", %{conn: conn, user: user, post: post} do
    {:ok, role} = TestHelper.create_role(%{name: "Admin", admin: true})
    {:ok, admin} = TestHelper.create_user(role, %{username: "admin", email: "admin@test.com", password: "test", password_confirmation: "test"})
      conn =
        login_user(conn, admin)
  ...
  test "when logged in as a different user, shows chosen resourse with author flag set to false", %{conn: conn, user: user, other_user: other_user, post: post} do
